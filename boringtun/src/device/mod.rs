...
impl<P: PeerRegistry> DeviceHandle<P>  {
    ...
    fn event_loop(_i: usize, device: &Lock<Device<P>>) {
        #[cfg(target_os = "linux")]
...
impl<P: PeerRegistry> Drop for DeviceHandle<P> {
    fn drop(&mut self) {
...
impl<P: PeerRegistry> Device<P> {
    ...
    pub fn new(name: &str, config: DeviceConfig<P>) -> Result<Device<P>, Error> {
        let poll = EventPoll::<Handler<P>>::new()?;
