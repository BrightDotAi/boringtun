...
fn api_get<P: PeerRegistry>(writer: &mut BufWriter<&UnixStream>, d: &Device<P>) -> i32 {
...
fn api_set<P: PeerRegistry>(
    reader: &mut BufReader<&UnixStream>,
    d: &mut LockReadGuard<Device<P>>,
) -> i32 {
    d.try_writeable(
...
fn api_set_peer<P: PeerRegistry>(
    reader: &mut BufReader<&UnixStream>,
    d: &mut Device<P>,
        ...
        }
        {
            ...
            match key {
                ...
                "public_key" => {
                    ...
                    //clear the vector content after update
                    allowed_ips.clear();
                    match val.parse::<KeyBytes>() {
                        Ok(key_bytes) => return api_set_peer::<P>(reader, d, key_bytes.0.into()),
                        Err(_) => return EINVAL,
                    }
                }
